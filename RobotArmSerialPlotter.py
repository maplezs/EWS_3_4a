...
VERSION = '1.19.1'
TIMESCALESIZE = 2000  # = self.plot_timescale and self.plot_data_size
...
pg.setConfigOption('background', colors['white'])
pg.setConfigOption('foreground', colors['black'])
...
class Plot(pg.GraphicsLayoutWidget):
    ...
    def __init__(self, nr_plot_lines='1', labels=["sensor1"]):
        ...
        labelStyle = {'color': '#000000', 'font-size': '30pt'}
        ...
        for i in range(self.nr_plot_lines):
            ...
            pen = pg.mkPen(color=(plot_colors[color_i]), width=5)
...
class MainWindow(QWidget):
    ...
    def __init__(self, app, parent=None):
        ...
        self.labels = ["label"]
        self.ports = ['']  # list of available devices
        ...
        self.menu_data_input = QHBoxLayout()
        ...
        self.data_input.setStyleSheet(QLineEdit_style)
        self.send_button = QtWidgets.QPushButton()
        self.send_button.setStyleSheet(SendButton_style)
        self.send_button.setText("Send")
        self.menu_data_input.addWidget(self.data_input)
        self.menu_data_input.addWidget(self.send_button)
        self.controls_layout.addLayout(self.menu_data_input)
        self.controls_layout.setAlignment(self.menu_data_input, Qt.AlignmentFlag.AlignBottom)
    ...
    def send_data(self):
        sentData = self.data_input.text()
        self.iterationLen = json.loads(sentData)
        self.iterationLen = self.iterationLen.get('thetaLen')
        sentData = sentData + '\n'
        self.ser.write(sentData.encode())
        temp = self.ser.readline()
        temp = temp.decode()
        if "data ok" in temp:
            logging.debug("data received ok")
            time.sleep(0.5)
            pass
        else:
            print("data not ok")
        if not self.plot_exist:
            ...
            if self.number_of_lines is not None:
                # Init plot
                self.labels = ["Reference 1", "Actual 1", "Reference 2", "Actual 2", "Reference 3", "Actual 3"]
                # self.labels = ["Reference 1", "Actual 1"]
                # self.labels = ["Reference 2", "Actual 2"]
                # self.labels = ["Reference 3", "Actual 3"]
                self.plot = Plot(self.number_of_lines, self.labels)
                self.controls_layout.insertWidget(0, self.plot)
                self.plot_exist = True
                for i in range(self.iterationLen):
                    self.timer.timeout.connect(self.monitor_data)
            else:
    ...
    def thread_find_ports(self):
        ...
        self.worker = ForeverWorker(self.find_ports)
    ...
    def init_ui(self):
        ...
        self.setWindowTitle("Robot Arm Serial Plotter")
    ...
    def connect_stop(self):
        ...
        if not self.is_button_connected:
            ...
            self.is_button_connected = False
            logging.debug('--> Disconnect Button.')
            self.disconnect()
    ...
    def init_connection(self):
        ...
        self.controls.connect.setText('Disconnect')
        ...
        try:
            ...
            tdata = tdata.decode()
            if tdata == "sync_ok\n":
                logging.debug("Success -- Open serial: %s %s", self.ser.name, self.ser.baudrate)
    ...
    def clear_data(self):
        ...
        self.plot.x_axis[0] = 0
        for i in range(self.number_of_lines):
            del self.plot.y_axis[i][0:(size - 1)]
            self.plot.y_axis[i][0] = 0
    ...
    def about(self):
        ...
        self.aboutbox.setText("<center></center><b>Robot Arm Serial Plotter</b><br/><br/>\
            <a href ='https://github.com/maplezs/EWS_3_4a'>\
            https://github.com/maplezs/EWS_3_4a</a><br/><br/>\
            Based on: <a href ='https://github.com/taunoe/tauno-serial-plotter'>\
            github.com/taunoe/tauno-serial-plotter</a><br/><br/>\
            Originally developed by Tauno Erik<br/><br/>\
            Modified by Rafsanjani Nurul Irsyad<br/><br/>\
            2024")
        self.aboutbox.exec()
    ...
    #         tdata = tdata.decode()
    #         print(tdata)
    #         if "done" not in tdata:
    #             # logging.info("serial_data: %s", tdata)
    #             numbers = self.get_numbers(tdata)
    #             # with open('..\\systemrespone.csv', mode='a', newline='') as employee_file:
    #             #     employee_writer = csv.writer(employee_file)
    #             #     employee_writer.writerow(numbers)
    #
    ...
    def monitor_data(self):
        try:
            # while self.four_count != 4:
            #     for i in range(5):
            #         self.four_count = i
            tdata = self.ser.readline()
            tdata = tdata.decode()
            print(tdata)
            words_to_check = ["referenceone", 'actualone', "referencetwo", 'actualtwo', "referencethree", 'actualthree']
            # words_to_check = ["referenceone", 'actualone']
            # words_to_check = ["referencetwo", 'actualtwo']
            # words_to_check = ["referencethree", 'actualthree']
            words_check = all(word in tdata.lower() for word in words_to_check)
            if "done" in tdata:
                print("done")
                self.timer.stop()
                return
